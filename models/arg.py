...
class ARGModel(torch.nn.Module):
    def __init__(self, config):
        ...
        self.bert_FTR = BertModel.from_pretrained(config["bert_path"]).requires_grad_(
            False
        )
        ...
        self.co_attention_2 = ParallelCoAttentionNetwork(
            config["emb_dim"], config["co_attention_dim"], mask_in=True
        )
        self.co_attention_3 = ParallelCoAttentionNetwork(
            config["emb_dim"], config["co_attention_dim"], mask_in=True
        )

        self.cross_attention_content_2 = SelfAttentionFeatureExtract(
            1, config["emb_dim"]
        )
        self.cross_attention_content_3 = SelfAttentionFeatureExtract(
            1, config["emb_dim"]
        )
    ...
    def forward(self, **kwargs):
        ...
        simple_ftr_2_pred = self.simple_mlp_ftr_2(
            self.simple_ftr_2_attention(FTR_2_feature)[0]
        ).squeeze(1)
        simple_ftr_3_pred = self.simple_mlp_ftr_3(
            self.simple_ftr_3_attention(FTR_3_feature)[0]
        ).squeeze(1)
    ...
    def train(self, logger=None):
        ...
        for epoch in range(self.config["epoch"]):
            ...
            for step_n, batch in enumerate(train_data_iter):
                batch_data = data2gpu(
                    batch, self.config["use_cuda"], data_type=self.config["data_type"]
                )
                ...
                loss_hard_aux = loss_hard_aux_fn(
                    res["hard_ftr_2_pred"], hard_ftr_2_label.float()
                ) + loss_hard_aux_fn(res["hard_ftr_3_pred"], hard_ftr_3_label.float())

                loss_simple_aux_fn = torch.nn.CrossEntropyLoss()
                loss_simple_aux = loss_simple_aux_fn(
                    res["simple_ftr_2_pred"], simple_ftr_2_label.long()
                ) + loss_simple_aux_fn(
                ...
                loss = loss_classify
                loss += (
                    self.config["model"]["rationale_usefulness_evaluator_weight"]
                    * loss_hard_aux
                    / self.num_expert
                )
                loss += (
                    self.config["model"]["llm_judgment_predictor_weight"]
                    * loss_simple_aux
                    / self.num_expert
                )
            ...
            # tensorlog
            writer.add_scalar(
                "month_" + str(self.config["month"]) + "/train_loss",
                avg_loss_classify.item(),
                global_step=epoch,
            )
            writer.add_scalars(
                "month_" + str(self.config["month"]) + "/test",
                results,
                global_step=epoch,
            )
        ...
        test_dir = os.path.join(
            "./logs/test/", self.config["model_name"] + "_" + self.config["data_name"]
        )
        os.makedirs(test_dir, exist_ok=True)
        test_res_path = os.path.join(
            test_dir, "month_" + str(self.config["month"]) + ".json"
        )

        self.model.load_state_dict(
            torch.load(os.path.join(self.save_path, "parameter_bert.pkl"))
        )
        test_future_results, label, pred, id, ae, acc = self.predict(test_future_loader)

        writer.add_scalars(
            "month_" + str(self.config["month"]) + "/test", test_future_results
        )
        ...
        if logger:
            ...
            logger.info(
                "lr: {}, avg test score: {}.\n\n".format(
                    self.config["lr"], test_future_results["metric"]
                )
            )
